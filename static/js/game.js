...
function initCheatMenu(_nick){
  // isTester выставляется через activateCheat() — не меняем здесь
...
/* ════════════ CHEAT ACTIVATION (по коду) ════════════ */
async function activateCheat(){
  isTester = true;
  const btn=$("snav-cheat"); if(btn) btn.classList.remove("snav-hidden");
...
function initAdminUI(){
  on($("btn-admin-activate"),"click",async()=>{
    ...
    // Сначала проверяем код чит-меню (CHEAT_CODE из .env)
    const cheatRes=await fetch("/verify_cheat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({key})}).then(r=>r.json()).catch(()=>({}));
    if(cheatRes.ok){
      $("admin-key-input").value="";
      $("admin-key-error").style.display="none";
      await activateCheat();
      return;
    }
    // Иначе — проверяем админ-ключ
    ...
    if(d.ok){ isAdmin=true; $("admin-key-section").style.display="none"; $("admin-panel-section").style.display=""; loadAdminRooms(); toast("✅ Администратор"); $("admin-key-input").value=""; }
