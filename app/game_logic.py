...
class Room:
    ...
    def recalculate_difficulty(self):
        """Адаптивная сложность: снижает сложность при >50% ошибок, повышает при >80% правильных."""
        ...
        if old_diff != self.current_difficulty:
            logger.info(f"🎯 Сложность изменена: {old_diff} → {self.current_difficulty} ({pct:.0%} правильных)")
...
rooms: dict = {}
# Обратный индекс sid -> room_code для O(1) поиска (вместо O(N*M))
_sid_room: dict = {}

ROOM_IDLE_TIMEOUT  = 90   # сек: комната без действий удаляется
ROOM_EMPTY_TIMEOUT = 30   # сек: пустая комната удаляется быстрее

def cleanup_stale_rooms() -> int:
    """Удаляет пустые и давно неактивные комнаты. Возвращает число удалённых."""
    now = int(time.time())
    to_delete = []
    for code, room in rooms.items():
        age = now - room.last_activity
        if not room.players:
            if age > ROOM_EMPTY_TIMEOUT:
                to_delete.append(code)
        elif room.state == "waiting" and age > ROOM_IDLE_TIMEOUT:
            to_delete.append(code)
...
def get_room_by_sid(sid: str):
    code = _sid_room.get(sid)
    return rooms.get(code) if code else None

def index_add(sid: str, code: str):
    _sid_room[sid] = code

def index_remove(sid: str):
    _sid_room.pop(sid, None)
