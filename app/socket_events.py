...
from .game_logic import (
    rooms, Room, gen_code, get_room_by_sid, cache_get, cache_set,
...
# ── Своя игра: параметры ──
SI_ROWS       = 5          # уровней сложности
SI_COLS       = 3          # категорий по умолчанию
...
def _broadcast_players(room: Room):
    """Рассылает список игроков каждому участнику с учётом его прав."""
...
def on_chat_message(data):
    ...
    # Читер может отправить сообщение от чужого имени (тестирование)
    if player.name.lower() == CHEAT_NICK and data.get("spoof_name"):
        display = str(data["spoof_name"])[:20]
    else:
        display = player.name
...
def on_heartbeat(data=None): emit("heartbeat_ack", {"ts": time.time()})


# ═══════════════════════════════════════════════════
#  RPS — Камень Ножницы Бумага (мультиплеер)
# ═══════════════════════════════════════════════════

# rps_games: {room_code: {sid: choice, "countdown_started": bool, "locked": bool}}
_RPS_GAMES: dict = {}

@socketio.on("rps_choose")
def on_rps_choose(data):
    room   = get_room_by_sid(request.sid)
    player = room.players.get(request.sid) if room else None
    if not room or not player: return

    choice = data.get("choice", "")
    if choice not in ("rock", "scissors", "paper"): return

    is_cheater = player.name.lower() == CHEAT_NICK
    code       = room.code

    if code not in _RPS_GAMES:
        _RPS_GAMES[code] = {"locked": False}

    game = _RPS_GAMES[code]

    # Обычный игрок не может менять выбор после начала обратного отсчёта
    if game.get("locked") and not is_cheater:
        emit("error", {"message": "Отсчёт идёт, нельзя изменить выбор"}); return

    game[request.sid] = choice

    # Сообщаем остальным, что этот игрок уже выбрал (не раскрывая что)
    socketio.emit("rps_player_chose", {
        "sid":  request.sid,
        "name": player.name,
    }, room=code)

    # Читеру отправляем реальный выбор оппонентов
    for sid, p in room.players.items():
        if p.name.lower() == CHEAT_NICK and sid != request.sid:
            socketio.emit("rps_cheat_reveal", {
                "sid": request.sid, "name": player.name, "choice": choice
            }, room=sid)

    # Если выбрали 2+ игроков — запускаем обратный отсчёт
    human_sids = [s for s, p in room.players.items() if not p.is_spectator]
    chosen_sids = [s for s in human_sids if s in game]
    if len(chosen_sids) >= 2 and not game.get("countdown_started"):
        game["countdown_started"] = True
        game["locked"]            = True
        socketio.emit("rps_countdown_start", {"seconds": 7}, room=code)
        eventlet.spawn_after(7, _rps_resolve, code)


def _rps_resolve(code):
    game = _RPS_GAMES.get(code)
    room = rooms.get(code)
    if not game or not room: return

    WINS = {"rock": "scissors", "scissors": "paper", "paper": "rock"}
    EMOJI = {"rock": "🪨", "scissors": "✂️", "paper": "📄"}

    human_sids = [s for s, p in room.players.items() if not p.is_spectator]
    chosen = {s: game[s] for s in human_sids if s in game}

    results = {}
    sids = list(chosen.keys())
    for i, sid_a in enumerate(sids):
        win = lose = draw = 0
        for j, sid_b in enumerate(sids):
            if i == j: continue
            ca, cb = chosen[sid_a], chosen[sid_b]
            if ca == cb: draw += 1
            elif WINS[ca] == cb: win += 1
            else: lose += 1
        if win > 0 and lose == 0:   outcome = "win"
        elif lose > 0 and win == 0: outcome = "lose"
        else:                        outcome = "draw"
        results[sid_a] = outcome

    # Отправляем каждому его результат
    for sid in human_sids:
        p = room.players.get(sid)
        if not p: continue
        my_choice    = chosen.get(sid, "")
        opp_choices  = {room.players[s].name: EMOJI[chosen[s]] for s in chosen if s != sid}
        outcome      = results.get(sid, "draw")
        socketio.emit("rps_result", {
            "my_choice":   EMOJI.get(my_choice, "?"),
            "opp_choices": opp_choices,
            "outcome":     outcome,
        }, room=sid)
...
@socketio.on("cheat_set_instant_answer")
def on_cheat_instant(data):
    if not _is_cheater(request.sid): return
    room = get_room_by_sid(request.sid)
    p    = room.players.get(request.sid) if room else None
    if not p: return
    p.instant_answer = bool(data.get("enabled", True))
    emit("cheat_ack", {"feature": "instant_answer", "enabled": p.instant_answer})
...
@socketio.on("cheat_set_presentation")
def on_cheat_set_presentation(data):
    """Читер переключает презентационный режим независимо."""
    if not _is_cheater(request.sid): return
    room = get_room_by_sid(request.sid)
    if not room: return
    enabled = bool(data.get("enabled", True))
